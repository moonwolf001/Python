# (c)2024 MoonWolf（むーんうるふ） // Python pygameにて花火大会 Part2
# Part2では、粒が3から1となり繊細な花火。表示時間は少し縮小。
...
# 花火の設定
FIREWORK_RADIUS_MULTIPLIERS = [20, 13, 25]  # 各破裂段階の初速度係数
FIREWORK_TIMINGS = [500, 300, 700]  # 各段階の破裂タイミング (ミリ秒)
FIREWORK_PARTICLE_COUNTS = [200, 250, 400]  # 各段階のパーティクル数
PARTICLE_LIFE_SPANS = [4, 2, 2]  # 各破裂段階のパーティクル寿命 (秒)
PARTICLE_GRAVITY = 40  # パーティクルに作用する重力
PARTICLE_FRICTION = 0.970  # パーティクルの速度減衰率
