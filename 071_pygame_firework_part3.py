# (c)2024 MoonWolf（むーんうるふ） // Python pygameにて花火大会 Part3
# Part3では、粒半径が3から5。色を調整。
...
SCREEN_TITLE = "Fireworks Show Part2 by MoonWolf, 2024"
...
# 花火の表示数のシーケンス設定
FIREWORK_SEQUENCE = [(0, 3), (7000, 5), (14000, 10), (20000, 20),(30000, 30), (40000,0)]  # (開始時間ms, 表示最大数)

# 花火の設定
FIREWORK_RADIUS_MULTIPLIERS = [20, 13, 35]  # 各破裂段階の初速度係数
FIREWORK_TIMINGS = [500, 300, 700]  # 各段階の破裂タイミング (ミリ秒)
FIREWORK_PARTICLE_COUNTS = [200, 250, 400]  # 各段階のパーティクル数
PARTICLE_LIFE_SPANS = [2, 2, 2]  # 各破裂段階のパーティクル寿命 (秒)
PARTICLE_GRAVITY = 49  # パーティクルに作用する重力
PARTICLE_FRICTION = 0.950  # パーティクルの速度減衰率

# 星の設定
STAR_COUNT = 20  # 星の数
STAR_SIZE_MIN = 1 # 星の最小サイズ
STAR_SIZE_MAX = 1  # 星の最大サイズ

# 色の設定
COLORS = {
    'Color1': ['ghostwhite', 'whitesmoke', 'gainsboro', 'lightcyan', 'dodgerblue'],
    'Color2': ['orangered', 'coral', 'orchid', 'lime', 'lightsalmon'],
    'Color3': ['khaki', 'lightyellow', 'ivory', 'ghostwhite', 'lavender']
}
...
# 花火リストとタイマーの初期化
fireworks = []
next_firework_timing = pygame.time.get_ticks() + random.randint(100, 300)
