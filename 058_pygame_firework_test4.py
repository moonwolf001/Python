#(c)2024 MoonWolf // pygame firework testing
import pygame
import random
import math
...
FIREWORK_Y_RANGE = (80, HEIGHT // 2)  # Y座標の破裂範囲（画面の上半分）

# 花火の設定
MAX_FIREWORKS_DISPLAYED = 5  # 画面に同時に表示する花火の最大数
FIREWORK_RADIUS_MULTIPLIERS = [5, 3, 12]  # 各破裂段階の初速度係数
FIREWORK_TIMINGS = [500, 300, 600]  # 各段階の破裂タイミング (ミリ秒)
FIREWORK_PARTICLE_COUNTS = [100, 100, 400]  # 各段階のパーティクル数
...
# 星の生成
stars = [(random.randint(0, WIDTH), random.randint(0, HEIGHT // 2), random.randint(STAR_SIZE_MIN, STAR_SIZE_MAX)) for _
         in range(STAR_COUNT)]
...
while running:
    ...
    if len(fireworks) < MAX_FIREWORKS_DISPLAYED and current_time > next_firework_timing:
        ...
        next_firework_timing = current_time + random.randint(500, 1500)
