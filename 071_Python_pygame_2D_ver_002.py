...
# pygame レトロっぽい横スクロール２Dゲーム, Version 0.02
...
pygame.display.set_caption("レトロっぽい横スクロール２Dゲーム by MoonWoolf, 2024")  # ウィンドウのタイトルを設定
...
class Enemy(pygame.sprite.Sprite):
    ...
    def draw(self):
        ...
        for start, end in edges:
            pygame.draw.line(screen, self.color, projected[start], projected[end], 4) # line width org 2
...
while running:
    ...
    for enemy in enemies:
        if check_player_collision(player, enemy):
            ...
            score_display = font.render(f'Your Score: {score}', True, (000, 100, 255))
    ...
    if game_over:
        # ゲームオーバー後の処理
        text_restart = font.render('Hit Return Key to Restart', True, (000, 155, 255))
