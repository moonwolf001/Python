#(c)2024 MoonWolf 弾、自機への当たり判定
#自分の機体への当たり判定ごGameOver。２秒後アプリ終了。
...
def check_collision(missile, enemy):
    """ Check collision between a missile and an enemy """
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        if event.type == pygame.KEYDOWN and event.key is pygame.K_SPACE and not game_over:
            if shoot_timer <= 0:
    ...
    if game_over:
        pygame.time.wait(2000)  # Show the game over screen for 2 seconds
        break
